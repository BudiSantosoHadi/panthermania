...
@section('container')
    ...
    <div class="g-white p-5 rounded">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Update DATA BPO</h1>
        </div>
        <hr>
            <form action="{{ route('Admin.update.bpo', $data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3 mt-3 row">
                    ...
                    <div class="col-sm-9">
                        <select class="form-control @error('data_penasehat') is-invalid @enderror" value="{{ old('data_penasehat') }}" type="text" name="data_penasehat" id="data_penasehat" required>
                            <option value="{{ old('data_penasehat', $data->data_penasehat) }}" disabled selected>{{ old('data_penasehat', $data->data_penasehat) }}</option>
            ...
            <div class="mb-3 mt-3 row">
                ...
                <div class="col-sm-9">
                    <select class="form-control @error('data_pembina') is-invalid @enderror" value="{{ old('data_pembina') }}" type="text" name="data_pembina" id="data_pembina" required>
                        <option value="{{ old('data_pembina', $data->data_pembina) }}" disabled selected>{{ old('data_pembina', $data->data_pembina) }}</option>
                        @foreach ($member as $members)
                            <option value="[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}">[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}</option>
                        @endforeach
                    </select>
                    ...
                    <div class="mb-3 mt-3 row">
                        ...
                        <select class="form-control @error('data_ketua_umum') is-invalid @enderror" value="{{ old('data_ketua_umum') }}" type="text" name="data_ketua_umum" id="data_ketua_umum" required>
                            <option value="{{ old('data_ketua_umum', $data->data_ketua_umum) }}" disabled selected>{{ old('data_ketua_umum', $data->data_ketua_umum) }}</option>
                            @foreach ($member as $members)
                                <option value="[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}">[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}</option>
                            @endforeach
                        </select>
                        ...
                        <div class="mb-3 mt-3 row">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control @error('data_wakil_ketua_umum') is-invalid @enderror" value="{{ old('data_wakil_ketua_umum') }}" type="text" name="data_wakil_ketua_umum" id="data_wakil_ketua_umum" required>
                                    <option value="{{ old('data_wakil_ketua_umum',$data->data_wakil_ketua_umum) }}" disabled selected>{{ old('data_ketua_umum', $data->data_wakil_ketua_umum) }}</option>
                                    @foreach ($member as $members)
                                        <option value="[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}">[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3 mt-3 row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control @error('data_sekretaris_umum') is-invalid @enderror" value="{{ old('data_sekretaris_umum') }}"  type="text" name="data_sekretaris_umum" id="data_sekretaris_umum" required>
                                        <option value="{{ old('data_sekretaris_umum', $data->data_sekretaris_umum) }}" disabled selected>{{ old('data_sekretaris_umum', $data->data_sekretaris_umum) }}</option>
                                        @foreach ($member as $members)
                                            <option value="[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}">[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3 mt-3 row">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control @error('data_bendahara_umum') is-invalid @enderror" value="{{ old('data_bendahara_umum') }}"  type="text" name="data_bendahara_umum" id="data_bendahara_umum" required>
                                            <option value="{{ old('data_bendahara_umum',$data->data_bendahara_umum) }}" disabled selected>{{ old('data_bendahara_umum', $data->data_bendahara_umum) }}</option>
                                            @foreach ($member as $members)
                                                <option value="[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}">[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}</option>
                                            @endforeach
                                    </select>
                                        ...
                                        <div class="mb-3 mt-3 row">
                                            ...
                                            <div class="col-sm-9">
                                                <select class="form-control @error('data_kordinator_ibu_ibu') is-invalid @enderror" value="{{ old('data_kordinator_ibu_ibu') }}" type="text" name="data_kordinator_ibu_ibu" id="data_kordinator_ibu_ibu" required>
                                                    <option value="{{ old('data_kordinator_ibu_ibu',$data->data_kordinator_ibu_ibu) }}" disabled selected>{{ old('data_kordinator_ibu_ibu' , $data->data_kordinator_ibu_ibu) }}</option>
                                                    @foreach ($member as $members)
                                                        <option value="[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}">[{{ $members->no_punggung }}] {{ $members->nama_lengkap }}</option>
                                                    @endforeach
                                                </select>
            ...
            <a href="{{ route('Admin.index.bpo') }}" class="btn mt-4 mb-4 btn-danger">
        <span>batal</span></a>
        </form>
